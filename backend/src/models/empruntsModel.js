...
{
    const result = await pool.query( 'SELECT COUNT(*) FROM emprunt');
...
{
    const result = await pool.query( 'SELECT * FROM emprunt WHERE actif = true ORDER BY nom, prenom');
...
export const create = async ({ nom, prenom, email }) => 
{
    ...
    const result = await pool.query( 
        `INSERT INTO emprunt (numero_adherent, nom, prenom, email) VALUES ($1, $2, $3, $4) 
            RETURNING *`,
...
{
    const result = await pool.query(
        'UPDATE emprunt SET actif = false WHERE id = $1 RETURNING *',
        [id]
    );
